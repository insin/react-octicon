...
var App = React.createClass({
  ...
  render() {
    ...
    return <div className="App">
      ...
      <p>
        <select value={name} onChange={e => this.setState({name: e.target.value})}>
          {NAMES.map(name => <option>{name}</option>)}
        </select>{' '}
        <label><input type="checkbox" checked={mega} onChange={e => this.setState({mega: e.target.checked})}/> mega</label>{' '}
        <label><input type="checkbox" checked={spin} onChange={e => this.setState({spin: e.target.checked})}/> spin</label>
      </p>
